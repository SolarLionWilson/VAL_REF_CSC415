...
def position(tx, id):
    global  table;
    table[0] = tableValue(id, "TEST", "TEST", "TEST", "TEST") #mod
    i = tx
...
def enter(tx, k, level, dx):
    ...
    if k == "const":
        ...
        x = tableValue(id, k, level, dx, "NULL")
    elif k == "function": #mod at function ST type
        x = tableValue(id, k, level, dx, "NULL")
    table.append(x)
    return dx
#--------------CONST DECLARATION---------------------------
...
def block(tableIndex, level):
    ...
    while (t == 1):
        ...
        while sym == "PROCEDURE" or sym == "FUNCTION": #mod
            ...
            getsym()
            if sym == "ident":
                if tmpSym == "PROCEDURE": #mod
                    enter(tx, "procedure", level, codeIndx)
                if tmpSym == "FUNCTION": #mod
            ...
            if sym != "semicolon":
                error(10)
            getsym()
        if (sym == "PROCEDURE" or sym == "VAR" or sym == "CONST" or sym == "FUNCTION"):
            t = 1
        else:
            t = 0 #end mod restricted globals
    fixJmp(cx1, codeIndx)
    if tx0 != 0:
...
def statement(tx, level):
    ...
    if sym == "ident" or sym == "FUNCTION" or sym == "VAL" or sym == "REF": #mod
        i = position(tx, id)
        if i==0:
            error(11)
        else:
        ...
        if (tmpKind == "function"):
            if (i == (tx - funcMap[table[i].name])-table[i].nest): #mod
                gen("STO", 0, -1)
            else:
                error(38) #must be inside function body
        if (tmpKind == "ref"):
            gen("STI", level - table[i].level, table[i].adr)
    elif sym == "CALL": #begin mod
        getsym()
        if sym != "ident":
            error(14)
        i = position(tx, id)
        ...
        if (sym == "lparen"):
            ...
            if sym != "rparen":
                error(22)
            getsym()
        gen("CAL", level - table[i].level, table[i].adr) #end mod
    #begin mod
    elif sym == "IF":
        getsym()
    ...
    #begin mod	
        ...
        if (table[i].kind != "variable"): #sym must be a variable
            error(36)
        getsym()
        if (sym != "becomes"):
            error(13)
        ...
        if not (sym == "CEND"):
            error(37) #expected CEND keyword
        fixJmp(cx2, codeIndx)
        gen("INT", 0, -1)
        getsym() #end mod
    elif (sym == "WRITE" or sym == "WRITELN"): #begin mod
        tempSym = sym
        getsym()
        if (sym != "lparen"):
            error(27)
...
def expression(tx, level):
    ...
    while sym == "plus" or sym == "minus" or sym == "OR":
        ...
        if(addop == "minus"):
            ...
            gen("OPR", 0, 2)       #add operation and OR
#-------------TERM----------------------------------------------------
def term(tx, level):
    global sym;
    factor(tx, level)
    while sym=="times" or sym=="slash" or sym=="AND":
        ...
        if mulop == "times" or mulop == "AND":
            gen("OPR", 0, 4)         #multiply operation and AND
        else:
...
def factor(tx, level): #begin mod
    ...
    if sym == "ident" or sym == "VAL" or sym == "REF":
        ...
        if table[i].kind == "const":
            ...
            gen("LOD", level-table[i].level, table[i].adr)
        elif table[i].kind == "procedure" or table[i].kind == "function": #mod
            error(21)
        getsym()
    elif sym == "number":
        gen("LIT", 0, num)
...
rword.append('CALL')
rword.append('CONST')
rword.append('VAL') #mod
rword.append('REF') #mod
rword.append('DO')
rword.append('END')
rword.append('IF')
rword.append('ODD')
